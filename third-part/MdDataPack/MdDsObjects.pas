...
begin
  ...
  if FChangeToClone and Assigned (ObjClone) and
      (State = dsEdit) and
      (ActiveBuffer = fCurrentRecord) then
    Obj:= ObjClone
  else
    Obj := fList [ActiveBuffer] as TPersistent;
...
begin
  ...
  if FChangeToClone and Assigned (ObjClone) and
    ...
    Obj := fList [PRecInfo(ActiveBuffer).RecordNumber] as TPersistent;
  ...
  case TypeInfo.Kind of
    tkInteger, tkChar, tkWChar, tkClass, tkEnumeration, tkSet:
      begin
        IntValue := GetOrdProp(Obj, PropList [Field.FieldNo-1]);
        Move (IntValue, Buffer[0], sizeof (Integer));
      end;
    tkFloat:
      begin
        FlValue := GetFloatProp(Obj, PropList [Field.FieldNo-1]);
...
procedure TMdObjDataSet.SetFieldData(Field: TField; Buffer: TValueBuffer;
...
begin
  ...
  case TypeInfo.Kind of
    ...
    tkFloat:
      begin
        Move (Buffer[0], FlValue, sizeof (Double));
        SetFloatProp(Obj, PropList [Field.FieldNo-1], FlValue);
      end;
    tkString, tkLString, tkWString, tkUString:
